...
function CosmeticLib:Init()
	if not CosmeticLib.bHasInitialized then
		...
		-- Add console command
		Convars:RegisterCommand( "get_item_set_it_for_hero", function( cmd ) 
				return CosmeticLib:GetSetsIDForHero( Convars:GetCommandClient() )
			end, "Get set item for hero", 0
		)
		Convars:RegisterCommand( "equip_item_set", function( cmd, set_number ) 
				return CosmeticLib:EquipSetForHero( Convars:GetCommandClient(), set_number )
